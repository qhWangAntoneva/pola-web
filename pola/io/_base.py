"""
Dispatcher for pola.io adapters.
...
def _parse_markdown_tables(text: str) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    from pola.io._markdown_adapter import parse_markdown
...
def _lazy_load_ext(ext: str, module: str, func: str, buf_func: Optional[str] = None):
    ...
    def adapter(path):
        mod = __import__(f"pola.io._{module}", fromlist=[func])
    ...
    if buf_func:
        ...
        def buf_adapter(buf):
            mod = __import__(f"pola.io._{module}", fromlist=[buf_func])
